...
def createServerConfig():
    try:
        ...
        cmd(f"sudo sed -i 's/port 1194/port 2876/' {serverConfigPath}")
...
def commands():
    try:
        ...
        cmd("sudo ./easyrsa build-ca nopass")
        cmd("sudo ./easyrsa build-server-full server nopass")
...
def createIptables():
    try:
        ...
        cmd('sudo iptables -A INPUT -j ACCEPT -p tcp --dport 2244')
        cmd('sudo iptables -A INPUT -j ACCEPT -p udp --dport 2876')
...
def buildClientConfig():
    try:
        ...
        if os.path.exists(f"/usr/share/easy-rsa/pki/issued/{name}.crt"):
            ...
            cmd(f"sudo sed -i 's/remote my-server-1 1194/remote {ip} 2876/' {configFile}")
...
def hardenServer():
    try:
        ...
        cmd(f"sudo sed -i 's/#Port 22/Port 2244/' {sshConfPath}")
